...
export default function UserPostEditPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch(`/api/user/posts/${params.id}`).then(res => res.json() as Promise<{ success: boolean; data: { post: any } }>)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/user/posts/${params.id}`, {
        method: 'PUT',
        headers: {
      ...
      <Card>
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">编辑文章</h2>
