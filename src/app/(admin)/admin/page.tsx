    ...
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {recentPosts.map((post) => (
                  <li key={post.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">{post.author_name} · {new Date(post.created_at).toLocaleDateString('zh-CN')}</p>
                    </div>
        ...
        <Card>
          <CardContent className="p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">
              待审核评论 ({stats?.pendingComments || 0})
            </h3>
            {pendingComments.length > 0 ? (
              <ul className="space-y-3">
                {pendingComments.map((comment) => (
                  <li key={comment.id} className="py-3 border-b border-gray-100 last:border-0">
