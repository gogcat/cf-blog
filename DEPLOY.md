...
```jsonc
{
  ...
  "vars": {
    "SITE_URL": "https://your-domain.com",  ← 替换为你的网站地址
...
- `SITE_URL` - 必须替换为你的网站地址（如 `https://your-domain.com`）
- `SITE_NAME` - 可以自定义你的网站名称

**💡 说明**：
- `vars` 部分定义了环境变量
- 部署时，这些变量会自动应用到 Worker
- 避免了每次部署后环境变量被清除的问题
...
- `wrangler.jsonc` 中的 `vars` 环境变量会自动应用到 Worker
